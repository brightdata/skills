...
3. **Use the account's own rate, not the public price list.** Accounts on custom or pre-commit pricing pay negotiated rates. For Web Unlocker and SERP you can compute the real rate: ask `/zone/cost` for a period with `from` and `to`, then divide the `custom` bucket's `cost` by its request count. When the bucket carries both `reqs_unblocker` and `reqs_serp`, divide by their sum, because the cost is one combined figure for the zone (live). For Web Scraper API the record count is a read (`/datasets/v3/snapshots`, `dataset_size`) and the dollars are in the cost export, per dataset under `web_apis` (live) or per snapshot under `ws_api_snaps` where attributed, so compute the real per-record rate when the reads return data. When they do not, use the published price and say plainly that it is the list price, not this account's rate.
