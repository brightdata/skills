---
name: scraper-studio
description: "Build and run AI-generated Bright Data Scraper Studio collectors from the terminal with `bdata scraper create` and `bdata scraper run`. Use when the user wants to turn a URL and natural-language extraction request into a reusable collector, run an existing collector against one URL, or batch-scrape multiple URLs through one collector. Covers create, run, heal, approve, async polling, `--sync`, `--urls`, `--input-file`, `collector_id`, and `/dca/` Scraper Studio flows. Requires the Bright Data CLI."
---
